...
if (isset($_GET['action']) && $_GET['action'] == 'get_messages') {
    ...
    $query = "SELECT m.*, p.ga4_2_nome AS profissional_nome, c.ga4_1_nome AS cliente_nome 
              ...
              WHERE m.ga4_4_id_cliente = $cliente_id AND m.ga4_4_id_profissional = $profissional_id
              ORDER BY m.ga4_4_data_envio ASC";
    $result = mysqli_query($conn, $query);
    $mensagens = mysqli_fetch_all($result, MYSQLI_ASSOC);
    
    // Atualizar status das mensagens do profissional para "lida" quando o cliente visualiza
    $update_query = "UPDATE ga4_4_mensagens 
                    SET ga4_4_status_mensagem = 'lida' 
                    WHERE ga4_4_id_cliente = $cliente_id 
                    AND ga4_4_id_profissional = $profissional_id 
...
if (isset($_POST['action']) && $_POST['action'] == 'send_message') {
    ...
    $query = "INSERT INTO ga4_4_mensagens (ga4_4_id_cliente, ga4_4_id_profissional, ga4_4_mensagem, ga4_4_enviado_por, ga4_4_data_envio, ga4_4_status_mensagem) 
              VALUES ('$cliente_id', '$profissional_id', '$mensagem', 'cliente', NOW(), 'não entregue')";
    $result = mysqli_query($conn, $query);
...
if (isset($_POST['action']) && $_POST['action'] == 'update_status') {
    ...
    $query = "UPDATE ga4_4_mensagens SET ga4_4_status_mensagem = '$new_status' WHERE id = $message_id";
    $result = mysqli_query($conn, $query);
...
$query = "SELECT ga4_1_nome FROM ga4_1_clientes WHERE ga4_1_id = $cliente_id";
$result = mysqli_query($conn, $query);
$cliente = mysqli_fetch_assoc($result);
$cliente_nome = $cliente['ga4_1_nome'];

// Verifique se um novo profissional foi selecionado
if (isset($_POST['profissional_id']) && !empty($_POST['profissional_id'])) {
    $_SESSION['profissional_id'] = $_POST['profissional_id'];
}

// Obtenha o profissional selecionado da sessão
$profissional_id = isset($_SESSION['profissional_id']) ? $_SESSION['profissional_id'] : '';
...
$query = "SELECT m.*, p.ga4_2_nome AS profissional_nome, c.ga4_1_nome AS cliente_nome 
          ...
          JOIN ga4_2_profissionais p ON m.ga4_4_id_profissional = p.ga4_2_id
          JOIN ga4_1_clientes c ON m.ga4_4_id_cliente = c.ga4_1_id
          WHERE m.ga4_4_id_cliente = $cliente_id";

if ($profissional_id) {
    $query .= " AND m.ga4_4_id_profissional = $profissional_id";
...
$query .= " ORDER BY m.ga4_4_data_envio ASC";
$result = mysqli_query($conn, $query);
$mensagens = mysqli_fetch_all($result, MYSQLI_ASSOC);
?>
...
<body>
    <main class="container">
        ...
        <div class="user-info">
            ...
            <p>Você está logado como <strong><?php echo $cliente_nome; ?></strong></p>
        </div>
        
        <div class="select-container">
            <form method="POST" action="chat_cliente.php">
                <label for="profissional_id">Selecione um profissional para conversar:</label>
                <select name="profissional_id" id="profissional_id" onchange="this.form.submit()">
                    <option value="">Escolha um profissional</option>
                    <?php
                    $query = "SELECT ga4_2_id, ga4_2_nome FROM ga4_2_profissionais";
                    $result = mysqli_query($conn, $query);
                    while ($profissional = mysqli_fetch_assoc($result)) {
                        $selected = isset($_SESSION['profissional_id']) && $_SESSION['profissional_id'] == $profissional['ga4_2_id'] ? 'selected' : '';
                        echo "<option value='{$profissional['ga4_2_id']}' $selected>{$profissional['ga4_2_nome']}</option>";
                    }
                    ?>
                </select>
            </form>
        ...
        <div class="chat-box" id="chat-box">
            <?php if ($profissional_id): ?>
                <div id="mensagens-container">
                    <?php foreach ($mensagens as $mensagem): ?>
                        <div class="mensagem <?php echo ($mensagem['ga4_4_enviado_por'] == 'cliente') ? 'cliente' : 'profissional'; ?>" data-id="<?php echo $mensagem['ga4_4_id']; ?>">
                            <strong>
                                <?php 
                                if ($mensagem['ga4_4_enviado_por'] == 'cliente') {
                            ...
                            <em class="data">
                                ...
                                <?php if ($mensagem['ga4_4_enviado_por'] == 'cliente'): ?>
            ...
            <?php else: ?>
                <p class="select-prompt">Por favor, selecione um profissional para iniciar o chat.</p>
            <?php endif; ?>
        </div>
        
        <form id="mensagemForm" <?php echo !$profissional_id ? 'style="display:none;"' : ''; ?>>
...
function criarElementoMensagem(mensagem) {
    ...
    div.className = `mensagem ${mensagem.ga4_4_enviado_por == 'cliente' ? 'cliente' : 'profissional'}`;
    div.setAttribute('data-id', mensagem.ga4_4_id);
    
    const strong = document.createElement('strong');
    strong.textContent = mensagem.ga4_4_enviado_por == 'cliente' ? 
    ...
    // Adicionar ícone de status para mensagens enviadas pelo cliente
    if (mensagem.ga4_4_enviado_por == 'cliente') {
...
function buscarNovasMensagens() {
    if (!document.hidden && <?php echo !empty($profissional_id) ? 'true' : 'false'; ?>) {
        fetch('chat_cliente.php?action=get_messages&profissional_id=<?php echo $profissional_id; ?>')
