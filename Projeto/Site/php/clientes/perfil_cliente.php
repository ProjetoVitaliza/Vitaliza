...
$sql = "SELECT ga4_1_nome, ga4_1_email, ga4_1_tel, ga4_1_cpf, ga4_1_cep, ga4_1_sexo, ga4_1_nasc 
        FROM ga4_1_clientes WHERE ga4_1_id = ?";
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["action"]) && $_POST["action"] == "update_profile") {
    ...
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("sssii", $nome, $email, $telefone, $cep, $cliente_id);
    ...
    if ($stmt->execute()) {
        ...
        $cliente["ga4_1_nome"] = $nome;
        $cliente["ga4_1_email"] = $email;
        $cliente["ga4_1_tel"] = $telefone;
        $cliente["ga4_1_cep"] = $cep;
    } else {
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["action"]) && $_POST["action"] == "change_password") {
    ...
    $sql = "SELECT ga4_1_senha FROM ga4_1_clientes WHERE ga4_1_id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("i", $cliente_id);
    $stmt->execute();
    $result = $stmt->get_result();
    ...
    if (password_verify($senha_atual, $row['ga4_1_senha'])) {
        ...
        if ($nova_senha === $confirmar_senha) {
            ...
            if (strlen($nova_senha) >= 6) {
                ...
                $sql = "UPDATE ga4_1_clientes SET ga4_1_senha = ? WHERE ga4_1_id = ?";
                $stmt = $conn->prepare($sql);
                $stmt->bind_param("si", $nova_senha_hash, $cliente_id);
...
if (!empty($cliente["ga4_1_nasc"])) {
    $data_nascimento = date("d/m/Y", strtotime($cliente["ga4_1_nasc"]));
}
...
<header class="header">
    <nav>
        ...
        <div class="user-info">
            ...
            <span><?php echo $cliente["ga4_1_nome"]; ?></span>
            
          
            <a href="cliente_home.php" class="btn btn-outline btn-sm" style="margin-left: 15px;">
        ...
        <div class="profile-container">
            <div class="profile-header">
                ...
                <div class="profile-title">
                    <h2><?php echo $cliente["ga4_1_nome"]; ?></h2>
                    <p>Cliente <?php echo htmlspecialchars($config_valor[1]); ?></p>
                </div>
            </div>
            ...
            <div id="info" class="tab-content active">
                <form method="POST" action="perfil_cliente.php" class="profile-form">
                    ...
                    <div class="form-group">
                        ...
                        <input type="tel" id="telefone" name="telefone" value="<?php echo htmlspecialchars($cliente["ga4_1_tel"]); ?>">
                    </div>
                    
                    <div class="form-group">
                        <label for="cpf"><i class="fas fa-id-card"></i> CPF</label>
                        <input type="text" id="cpf" name="cpf" value="<?php echo htmlspecialchars($cliente["ga4_1_cpf"]); ?>" readonly>
                    </div>
                    
                    <div class="form-group">
                        <label for="sexo"><i class="fas fa-venus-mars"></i> Sexo</label>
                        <input type="text" id="sexo" name="sexo" value="<?php echo htmlspecialchars($cliente["ga4_1_sexo"]); ?>" readonly>
                    </div>
                    
                    <div class="form-group">
                        <label for="data_nascimento"><i class="fas fa-birthday-cake"></i> Data de Nascimento</label>
                        <input type="text" id="data_nascimento" name="data_nascimento" value="<?php echo $data_nascimento; ?>" readonly>
                    </div>
            ...
            <div id="security" class="tab-content">
                <form method="POST" action="perfil_cliente.php" class="profile-form" id="password-form">
